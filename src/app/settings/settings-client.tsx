...
export function SettingsClient({ initialName, email }: SettingsClientProps) {
  ...
  const handleSave = async () => {
    setSaving(true);
    // Simulate save delay — wire to Supabase profiles table later
  ...
  return (
    <div className="max-w-2xl space-y-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 h-11 bg-surface/20 border border-border/40 rounded-md px-3 flex items-center text-sm text-text-primary/60">
                  <Mail className="w-4 h-4 mr-2 text-text-primary/40" />
                  {email}
                </div>
      ...
      <Card className="bg-surface/50 border-border/50">
        <CardContent className="p-6 space-y-6">
          <h2 className="text-lg font-bold text-text-primary flex items-center gap-2">
            {darkMode ? <Moon className="w-5 h-5 text-primary" /> : <Sun className="w-5 h-5 text-yellow-400" />} Preferences
          </h2>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-text-primary/50 mt-0.5">Switch between dark and light mode</p>
            </div>
            <button
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-text-primary/50 mt-0.5">Receive learning reminders and updates</p>
              </div>
            </div>
            <button
              onClick={() => setNotifications(!notifications)}
              className={`relative w-14 h-7 rounded-full transition-colors ${
                notifications ? "bg-success" : "bg-surface border border-border"
              }`}
            >
              <div
                className={`absolute top-0.5 w-6 h-6 rounded-full bg-white shadow-md transition-transform ${
      ...
      <div className="flex justify-end">
        <Button
          variant="primary"
          className="px-8 shadow-md border-transparent"
          onClick={handleSave}
          disabled={saving}
        >
