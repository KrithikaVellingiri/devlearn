        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="p-6 sm:p-8 lg:p-12 overflow-y-auto w-full h-full max-w-[1600px] mx-auto flex flex-col xl:flex-row gap-10">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-6 xl:gap-8">
                <div className="w-full">
                  ...
                  <div className="mt-8 max-w-xl">
                    ...
                    <div className="flex justify-between items-center mt-3">
                      <span className="text-[9px] font-bold text-text-secondary tracking-widest uppercase">Path Progress</span>
                ...
                <div className="flex items-stretch gap-4">
                  <div className="flex-1 min-w-0 bg-surface/50 border border-border/50 rounded-xl p-5 flex flex-col items-center justify-center relative overflow-hidden group hover:border-border transition-colors">
                    <Flame className="w-6 h-6 text-[#FF6C37] mb-2" />
                    <span className="text-2xl font-bold text-text-primary leading-none mb-1">{currentStreak}</span>
                    <span className="text-[8px] font-bold text-text-secondary tracking-[0.2em] uppercase text-center">Day Streak</span>
                  </div>
                  <div className="flex-1 min-w-0 bg-surface/50 border border-border/50 rounded-xl p-5 flex flex-col items-center justify-center hover:border-border transition-colors">
                    <Library className="w-6 h-6 text-[#00F0FF] mb-2" />
                    <span className="text-2xl font-bold text-text-primary leading-none mb-1">{lessonsCompletedThisWeek}</span>
                    <span className="text-[8px] font-bold text-text-secondary tracking-[0.2em] uppercase text-center">Lessons/Week</span>
                  </div>
                  <div className="flex-1 min-w-0 bg-surface/50 border border-border/50 rounded-xl p-5 flex flex-col items-center justify-center hover:border-border transition-colors">
                    <Hourglass className="w-6 h-6 text-yellow-400 mb-2" />
                    <span className="text-2xl font-bold text-text-primary leading-none mb-1">{studyHoursTotal}</span>
                    <span className="text-[8px] font-bold text-text-secondary tracking-[0.2em] uppercase text-center">Study Hours</span>
                  </div>
                </div>
              </div>

              {/* Commitment History */}
              <div className="bg-surface border border-border/50 rounded-2xl p-8 shadow-none">
                ...
                <div className="overflow-x-auto overflow-y-hidden pb-4 custom-scrollbar">
                  ...
                  <div className="relative h-4 mt-4 text-[9px] font-bold text-text-secondary uppercase tracking-[0.2em] min-w-[1050px]">
                    {monthLabels.map((m, i) => (
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <Link href="/dashboard" className="text-[10px] font-bold text-text-secondary tracking-[0.2em] uppercase hover:text-text-primary cursor-pointer transition-colors">View All</Link>
                </div>
                  ...
                  {inProgressCourses.length > 0 ? inProgressCourses.map((course: any) => {
                    ...
                    return (
                      <Link key={course.id} href={`/courses/${course.id}`} className="block">
                        <div className="bg-surface border border-border/50 rounded-2xl overflow-hidden hover:border-border transition-colors flex flex-col md:flex-row group h-auto md:h-48 cursor-pointer shadow-none">
                          <div className="w-full md:w-64 h-48 md:h-full relative overflow-hidden flex-shrink-0 border-r border-border/50 bg-slate-900">
                            {course.image && <img src={course.image} alt={course.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-all duration-500" />}
                          </div>
                          <div className="p-8 flex-1 flex flex-col justify-center relative">
                            <div className="absolute top-6 right-8 rounded-full bg-text-primary/5 border border-border px-3 py-1 flex items-center justify-center">
                              <span className="text-[9px] font-bold text-text-secondary tracking-widest uppercase">{course.level}</span>
                            </div>
                            <h4 className="text-xl font-bold text-text-primary mb-2 pr-20">{course.title}</h4>
                            <p className="text-sm text-text-secondary mb-8 max-w-xl line-clamp-2">{course.description || "Mastering advanced patterns and implementations inside the modern ecosystem."}</p>

                            <div className="mt-auto w-full">
                              <div className="flex justify-between items-end mb-3">
                                <span className="text-[10px] font-bold text-text-secondary tracking-widest uppercase">{course.completedLessonsCount} / {course.totalLessons} Lessons Completed</span>
                    ...
                    <div className="text-center py-16 bg-surface border border-border/50 rounded-2xl shadow-none">
                      <BookOpen className="w-8 h-8 text-text-primary/20 mx-auto mb-4" />
                      <p className="text-text-secondary text-sm font-semibold mb-1">No active courses</p>
                      <p className="text-text-secondary text-xs">Enroll in a course to see your progress here.</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="w-full xl:w-80 shrink-0 flex flex-col gap-8">
              ...
              {/* Weekly Objective */}
              <div className="bg-surface border border-border/50 rounded-2xl p-8 flex flex-col items-center text-center shadow-none relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[#5A4AF4]/5 rounded-full blur-2xl -mr-10 -mt-10"></div>

                <h4 className="text-[11px] font-bold text-text-secondary tracking-[0.2em] uppercase mb-10 w-full text-left">Weekly Objective</h4>

                <div className="relative w-40 h-40 flex items-center justify-center mb-8">
                  {/* SVG Radial Progress */}
                  <svg className="w-full h-full transform -rotate-90">
                    <circle cx="80" cy="80" r="70" fill="transparent" stroke="#222738" strokeWidth="12" />
                    <circle cx="80" cy="80" r="70" fill="transparent" stroke="#5A4AF4" strokeWidth="12" strokeDasharray="439.8" strokeDashoffset={439.8 - (439.8 * (weeklyPercent / 100))} strokeLinecap="round" className="transition-all duration-1000" />
                  </svg>
                ...
                <p className="text-xs text-text-secondary leading-relaxed mb-6">Maintain your momentum to unlock the &apos;Consistency&apos; badge this week.</p>
              ...
              <div>
                <h4 className="text-[11px] font-bold text-text-secondary tracking-[0.2em] uppercase mb-6 px-1">Achievements</h4>

                <div className="bg-surface border border-border/50 rounded-2xl p-6 flex flex-col gap-6 shadow-none">
                  ...
                  {unlockedAchievements.length > 0 ? (
                      ...
                      {unlockedAchievements.slice(0, 3).map((a) => (
                        <div key={a.id} className="flex items-center gap-5 group cursor-pointer">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[9px] font-bold text-text-secondary uppercase tracking-widest mt-1">Unlocked</span>
                          </div>
                        </div>
                      ))}
                    </>
                    ...
                    <div className="text-center py-6">
                      ...
                      <p className="text-xs text-text-secondary">No achievements unlocked yet.</p>
                  ...
                  <div className="w-full border-t border-border/50 pt-6 mt-2">
                    <Link href="/achievements" className="block text-center text-[10px] font-bold text-text-secondary tracking-[0.2em] uppercase hover:text-text-primary cursor-pointer transition-colors">
