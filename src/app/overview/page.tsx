      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-12">
          <div className="mb-12">
            ...
            <p className="text-text-secondary text-sm">Your learning activity at a glance.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-12">
            <Card className="bg-surface/50 border-border/50"><CardContent className="p-6 flex items-center gap-4"><div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><BookOpen className="w-6 h-6 text-primary" /></div><div><p className="text-[10px] font-bold text-text-secondary tracking-wider uppercase mb-1">Enrolled</p><p className="text-3xl font-bold text-text-primary">{totalEnrolled}</p></div></CardContent></Card>
            <Card className="bg-surface/50 border-border/50"><CardContent className="p-6 flex items-center gap-4"><div className="w-12 h-12 rounded-xl bg-cyan-400/10 flex items-center justify-center shrink-0"><CheckCircle className="w-6 h-6 text-cyan-400" /></div><div><p className="text-[10px] font-bold text-text-secondary tracking-wider uppercase mb-1">Completed</p><p className="text-3xl font-bold text-text-primary">{totalCompleted}</p></div></CardContent></Card>
            <Card className="bg-surface/50 border-border/50"><CardContent className="p-6 flex items-center gap-4"><div className="w-12 h-12 rounded-xl bg-success/10 flex items-center justify-center shrink-0"><TrendingUp className="w-6 h-6 text-success" /></div><div><p className="text-[10px] font-bold text-text-secondary tracking-wider uppercase mb-1">Progress</p><p className="text-3xl font-bold text-text-primary">{overallProgress}%</p></div></CardContent></Card>
            <Card className="bg-surface/50 border-border/50"><CardContent className="p-6 flex items-center gap-4"><div className="w-12 h-12 rounded-xl bg-warning/10 flex items-center justify-center shrink-0"><Activity className="w-6 h-6 text-warning" /></div><div><p className="text-[10px] font-bold text-text-secondary tracking-wider uppercase mb-1">This Week</p><p className="text-3xl font-bold text-text-primary">{weeklyLessons} <span className="text-sm font-bold text-text-secondary">lessons</span></p></div></CardContent></Card>
          </div>
            ...
            <section className="mb-12">
              ...
              <Card className="bg-surface border-border/50 overflow-hidden">
                <div className="flex flex-col md:flex-row">
                  {continueCourse.image && <div className="w-full md:w-64 h-48 md:h-auto relative overflow-hidden bg-slate-900 shrink-0"><img src={continueCourse.image} alt={continueCourse.title} className="absolute inset-0 w-full h-full object-cover" /></div>}
                  <CardContent className="p-6 flex flex-col justify-center flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-text-secondary mb-4"><span>{continueCourse.cc} / {continueCourse.tl} lessons</span><span className="text-cyan-400 font-bold">{continueCourse.pct}%</span></div>
          ...
          <section>
            ...
            {recentActivity.length === 0 ? (
              <Card className="bg-surface/30 border-border/50 border-dashed"><CardContent className="flex flex-col items-center justify-center py-16 gap-3"><Clock className="w-8 h-8 text-text-secondary mb-2" /><h3 className="text-lg font-bold text-text-primary">No activity yet</h3><p className="text-text-secondary text-sm text-center max-w-md">Start learning to see your recent completions here.</p></CardContent></Card>
            ) : (
              <div className="space-y-3">
                {recentActivity.map((item, idx) => (
                  <Card key={`${item.courseId}-${item.lessonId}-${idx}`} className="bg-surface border-border/50 hover:bg-surface-light transition-colors">
                    <CardContent className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-text-secondary mt-0.5">{item.completedAt}</p>
                      </div>
                    </CardContent>
                  </Card>
                ))}
